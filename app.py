...
# ─── MODEL CONFIG ────────────────────────────────────────────────────
MODEL = "claude-sonnet-4-20250514"   # Sonnet — much higher rate limits than Opus
MAX_RETRIES = 3
RETRY_WAIT_BASE = 10   # seconds — doubles each retry: 10s, 20s, 40s
INTER_FILE_DELAY = 4   # seconds to wait between consecutive PDF calls
...
def extract_insurance_data(pdf_bytes: bytes, api_key: str, filename: str = "") -> dict:
    """
    Extract insurance data from a PDF using Claude.
    Uses Sonnet (higher rate limits), with automatic retry on 429 errors.
    """
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if attempt < MAX_RETRIES - 1:
                ...
                st.warning(f"⏳ Rate limit{label} — αναμονή {wait}s (απόπειρα {attempt+1}/{MAX_RETRIES})...")
                time.sleep(wait)
            else:
                raise RuntimeError(
            ...
            raise RuntimeError(f"Claude API σφάλμα: {e.status_code} — {e.message}") from e

        except json.JSONDecodeError as e:
            raise RuntimeError(
                f"Το Claude επέστρεψε μη-έγκυρο JSON. "
...
def generate_pptx(client_name: str, client_members: list, proposals: list,
                  recommended_idx: int, broker_name: str, broker_tel: str,
                  broker_email: str) -> bytes:
    """
    Build a multi-slide PPTX comparison presentation.
    proposals: list of dicts with extracted insurance data.
    recommended_idx: 0-based index of the recommended proposal.
    """
    prs = Presentation()
    ...
    title_str = (
        f"{'Τρεις' if n_props == 3 else str(n_props)} Προτάσεις για {client_name}"
    )
    add_text(s, title_str, 0.4, 0.2, 12.5, 0.65,
             size=28, bold=True, color=C["white"])
    add_text(s, "Από την οικονομική επιλογή έως την ολοκληρωμένη διεθνή κάλυψη",
             0.4, 0.85, 12.5, 0.4, size=13, italic=True,
    ...
    for i, prop in enumerate(proposals):
        ...
        add_rect(s, xp, 1.4, card_w, 5.5, bg_col,
                 line_color=border, line_width=2 if is_rec else 0.8)
    ...
    for i, prop in enumerate(proposals):
        ...
        s.background.fill.solid()
        is_rec = (i == recommended_idx)
        s.background.fill.fore_color.rgb = (
            C["navyDark"] if is_rec else C["offWhite"]
        )
        ...
        label = (
            f"★ ΕΠΙΛΟΓΗ {chr(65+i)} — ΠΡΟΤΕΙΝΟΜΕΝΗ"
            if is_rec else f"ΕΠΙΛΟΓΗ {chr(65+i)}"
        )
        add_text(s, label, 0.5, 0.15, 7.0, 0.42,
                 size=10, bold=True, color=C["gold"] if is_rec else col)
        add_text(
            s,
            f"{prop.get('insurer', '')} — {prop.get('plan_name', '')}",
            0.5, 0.55, 10.0, 0.75,
            size=26, bold=True,
        ...
        for pi, (k, v) in enumerate(params):
            yp = 1.55 + pi * 0.52
            add_rect(s, 0.5, yp, 3.2, 0.42,
                     C["navy"] if not is_rec else rgb(0x1A, 0x35, 0x50))
            add_text(s, k, 0.5, yp, 3.2, 0.42,
                     size=10, bold=True,
                     color=C["white"] if not is_rec else C["gold"],
                     align=PP_ALIGN.CENTER)
            add_text(s, v, 3.8, yp + 0.05, 9.0, 0.42,
                     size=10,
                     color=C["textDark"] if not is_rec else rgb(0xC8, 0xDF, 0xF0))

        add_rect(s, 0.5, 3.75, 6.0, 0.4, C["green"])
        ...
        covers = [
            ...
            ("Εξωνοσοκ.",      (
                f"εως {sym}{prop.get('outpatient_limit', '—')} "
                f"({prop.get('outpatient_pct', '—')}%)"
                if prop.get("outpatient_limit")
                   and prop.get("outpatient_limit") != "Not Covered"
                else (prop.get("outpatient_limit", "—") or "—")
            )),
        ...
        for ci, (k, v) in enumerate(covers):
            yp = 4.25 + ci * 0.47
            tick = "✓" if ("Not Covered" not in str(v) and v not in ["—", None]) else "✗"
            tc = C["green"] if tick == "✓" else C["red"]
            add_text(s, tick, 0.55, yp, 0.4, 0.42,
                     size=12, bold=True, color=tc, align=PP_ALIGN.CENTER)
            add_text(s, k, 1.0, yp, 1.5, 0.42,
                     size=10, bold=True,
                     color=C["white"] if is_rec else C["navy"])
            add_text(s, v, 2.6, yp, 3.7, 0.42,
                     size=9,
        ...
        notes = prop.get("key_notes") or []
        notes_extra = [
            f"Αναμονή: {prop.get('waiting_period', 'Άμεση') or 'Άμεση'}",
            f"Προϋπ. παθήσεις: {prop.get('preexisting', '—') or '—'}",
        ]
        all_notes = list(notes) + notes_extra
    ...
    col_widths = [3.5] + [(13.33 - 0.8 - 3.5) / len(proposals)] * len(proposals)
    xstarts = [0.4]
    for cw in col_widths[:-1]:
        xstarts.append(xstarts[-1] + cw)

    header_colors = [C["navy"]] + [
        insurer_color(p.get("insurer", "")) for p in proposals
    ]
    ...
    row_data = [
        ...
        ("Εξωνοσ. Ιατροί/Εξετ.", lambda p: (
            f"εως {_sym(p)}{p.get('outpatient_limit','—')} "
            f"({p.get('outpatient_pct','—')}%)"
            if p.get("outpatient_limit")
               and p.get("outpatient_limit") != "Not Covered"
            else (p.get("outpatient_limit", "—") or "—")
        )),
    ...
    for ri, (label, fn) in enumerate(row_data):
        ...
        for ci, (xp, cw) in enumerate(zip(xstarts, col_widths)):
            ...
            if ci == 0:
                add_text(s, label, xp + 0.08, yp, cw - 0.1, 0.41,
                         size=9, color=C["navy"])
            else:
                val = fn(proposals[ci - 1])
                is_good = (
                    val not in ["Not Covered", "—", "null", None]
                    and "Not Covered" not in str(val)
                )
                display = (
                    str(val)
                    if val not in ["Not Covered", "null", None]
                    else "ΔΕΝ καλύπτει"
                )
                text_color = (
                    C["navy"] if "€" in display or "$" in display or "£" in display
                    else (C["green"] if is_good else C["red"])
                )
                add_text(s, display, xp + 0.05, yp, cw - 0.1, 0.41,
                         size=8.5, bold=is_rec_col,
    ...
    if recommended_idx < len(proposals):
        ...
        cur = rec.get("currency") or "EUR"
        sym = "€" if cur == "EUR" else ("$" if cur == "USD" else "£")
        rec_text = (
            f"{rec.get('insurer', '')} — {rec.get('plan_name', '')}  "
            f"|  {sym}{rec.get('annual_premium', '')} / έτος"
        )
        add_text(s, rec_text, 0.5, 1.0, 12.5, 0.6,
    ...
    for si, (tag, title, body) in enumerate(steps):
        ...
        add_text(s, tag, xp, 1.85, 4.0, 0.58,
                 size=12, bold=True, color=C["navy"], align=PP_ALIGN.CENTER)
        add_text(s, title, xp + 0.1, 2.55, 3.8, 0.55,
                 size=14, bold=True, color=C["teal"], align=PP_ALIGN.CENTER)
        add_text(s, body, xp + 0.15, 3.15, 3.7, 1.15,
    ...
    add_text(s, quote, 0.7, 4.65, 12.0, 1.05,
             size=13, italic=True, color=rgb(0xC8, 0xDF, 0xF0))
...
def main():
    ...
    with st.sidebar:
        ...
        client_name = st.text_input(
            "Επώνυμο / Όνομα Πελάτη", placeholder="π.χ. Τοτικίδη Κατία"
        )
    ...
    st.info(
        "Φόρτωσε 2–4 PDF προσφορές. "
        "Το Claude θα εξάγει αυτόματα όλα τα στοιχεία.",
        icon="ℹ️"
    )
    ...
    if st.button("🤖 Ανάλυση με Claude API", type="primary", disabled=not api_key):
        ...
        for idx, uf in enumerate(uploaded_files):
            progress.progress(
                idx / total,
                text=f"Ανάλυση {idx+1}/{total}: {uf.name}..."
            )
            try:
                data = extract_insurance_data(uf.read(), api_key, filename=uf.name)
    ...
    if st.session_state.get("proposals"):
        ...
        insurer_labels = [
            f"{p.get('insurer', '')} — {p.get('plan_name', '')} (€{p.get('annual_premium', '')})"
        ...
        st.info(
            "👆 Πάτα 'Ανάλυση με Claude API' για να εξαχθούν "
            "τα στοιχεία από τα PDFs."
        )
